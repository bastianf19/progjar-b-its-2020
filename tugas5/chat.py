...
class Chat:
    ...
    def proses(self, data):
        ...
        try:
            ...
            if (command == 'auth'):
                ...
                sessionid = j[1].strip()
                username = self.sessions[sessionid]['username']
                logging.warning("SHOWUSERACTIVE: {}" . format(sessionid))
                return self.get_showuseractive(username)
            elif (command == 'logout'):
                sessionid = j[1].strip()
                username = self.sessions[sessionid]['username']
                logging.warning("LOGOUT: {}" . format(sessionid))
                return self.get_logout(username, sessionid)
            else:
    ...
    def get_showuseractive(self, username):
        pesan = {}
        pesan['users'] = []
        for users in self.sessions.keys():
            pesan['users'].append(
                {'username': self.sessions[users]['username']})
    ...
    def autentikasi_user(self, username, password):
        ...
        self.sessions[tokenid] = {'username': username,
    ...
    def get_logout(self, username, sessionid):
